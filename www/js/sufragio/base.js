...
function set_constants(data) {
    ...
    return new Promise((resolve, reject) => {
         ...
         */
        const promise_templates_sufragio = load_templates_sufragio(
            constants.templates
        );

        Promise.all([promise_popular_html, promise_templates_sufragio])
            .then(() => {
                const promise_patio = load_patio();
                const promise_colores = fetch_template("colores");
                const promise_extension_js = load_extension_js(
                    constants.flavor
                );
                const promise_controller_js = load_extension_controller_js(
                    constants.flavor
                );
                Promise.all([
                    promise_colores,
                    promise_extension_js,
                    promise_controller_js,
                    promise_patio,
                ]).then(
                    ([
                        template_colores,
                        ...
                        if (constants.mostrar_cinta_demostracion) {
                            document.querySelector(
                                "#barra_opciones .cinta"
                            ).style.display = "block";
                        }
                        if (constants.mostrar_cinta_capacitacion) {
                            document.querySelector(
                                "#encabezado .cinta-capacitacion"
                            ).style.display = "block";
                        }
    ...
    return new Promise((resolve, reject) => {
        if (constants.templates_compiladas) {
            const url_templates_flavor =
                constants.PATH_TEMPLATES_FLAVORS +
                constants.flavor +
                "/sufragio/templates.html";
            const url_templates_sufragio =
                constants.PATH_TEMPLATES_VAR + "sufragio.html";
            Promise.all([
...
function load_css(flavor) {
    ...
    elem.rel = "stylesheet";
    elem.href =
        constants.PATH_TEMPLATES_FLAVORS + flavor + "/sufragio/flavor.css";
...
function load_extension_js(flavor) {
    return new Promise((resolve) => {
        const extensiones_js = ["template-" + flavor + ".js"];
        const promises_extensiones_js = extensiones_js.map((file) =>
            load_js(constants.PATH_EXTENSION_JS_MODULO + "/" + file)
        );
...
function load_extension_controller_js(flavor) {
    return new Promise((resolve) => {
        const extensiones_js = ["localController-" + flavor + ".js"];
        const promises_extensiones_js = extensiones_js.map((file) =>
            load_js(constants.PATH_EXTENSION_JS_MODULO + "/" + file)
        );
        Promise.all(promises_extensiones_js).then(() => {
    ...
    document.body.addEventListener(
        ...
        function (event) {
            if (last_click === null) {
                ...
                if (
                    (event.x <= 0 && event.y <= 0) ||
                    (dx < minXDiff && dy < minYDiff && timeDiff < minTimeDiff)
                ) {
